...
def get_last_command_and_run():
  # Paths for common history files
  possible_history_files = [
      '~/.zsh_history',  # Default Zsh history file
      '~/.bash_history'  # Default Bash history file
  ]
  ...
  for history_file in possible_history_files:
    ...
    try:
      with open(history_path, 'r', encoding='latin-1',
                errors='ignore') as file:
        # Reverse read the file to get the last relevant command
        for line in reversed(file.readlines()):
          if 'chip' not in line:
            # Attempt to extract the command based on common history formats
            last_command = line.split(
      ...
      continue  # Try the next history file if one is not found
    except Exception as e:
      return None, f"Error reading history file '{history_path}': {e}"

  if not last_command:
    return None, "No valid previous command found in any history files."

  # Execute the last command and capture the output
  try:
    result = subprocess.run(last_command,
                            shell=True,
    ...
    return None, f"Error executing command '{last_command}': {e}"
...
def analyze_error(context):
  ...
  detailed_analysis = client.chat.completions.create(
      messages=[{
          ...
          "system",
          "content":
          "You are an expert and technically adept at analyzing errors and suggesting fixes. If the user appears to be mistyping common terminal commands, suggest commands that might be the correct option"
      }, {
          "role": "user",
          "content": detailed_prompt
      }],
      model=model,
      max_tokens=detailed_response_length).choices[0].message.content.strip()
  ...
  preresponse1 = client.chat.completions.create(
      messages=[{
          ...
          "system",
          "content":
          "Based on the analysis, respond with 'Yes' or 'No' only."
      }, {
          "role": "user",
          "content": enhanced_context + initial_prompt
      }],
      model=model,
      max_tokens=1).choices[0].message.content.strip()
  ...
  if preresponse1.lower() == 'yes':
    ...
    preresponse2 = client.chat.completions.create(
        messages=[{
            ...
            "system",
            "content":
            "Provide just the error type. DO NOT WRITE A SENTENCE"
        }, {
            "role": "user",
            "content": enhanced_context + type_prompt
        }],
        model=model,
        max_tokens=6).choices[0].message.content.strip()
...
def ask_gpt(question):
  ...
  max_tokens = int(QUESTION_CONFIG.get('ResponseLength',
                                       200))  # Default to 200 if not set
  ...
  if hardcore_mode.lower() != 'yes':
    if len(sys.argv) > 1:
      if sys.argv[1] == "error":
        ...
        print(
            format_in_rectangle(initial_analysis_output,
                                color_code=preresponse1_color))
        ...
        print(format_in_rectangle("Setup: chip api <TOGETHER_API_KEY>"))
    ...
    if hardcore_mode.lower() == 'yes':
      if len(sys.argv) > 1:
        if sys.argv[1] == "error":
          ...
          command_color = '\033[36m'  # light blue
          print(commander)
          preresponse1, preresponse2, detailed_analysis = analyze_error(
              context)
          initial_analysis_output = f"Error detected: {preresponse1}\nType: {preresponse2}"
